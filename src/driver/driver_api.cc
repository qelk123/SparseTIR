...
Array<tvm::transform::Pass> CreatePassList(bool disable_loop_partition) {
  ...
  pass_list.push_back(tir::transform::HorizontalFusion());
  pass_list.push_back(tir::transform::UnifyThreadBinding());
  pass_list.push_back(tir::transform::InjectScope());
  pass_list.push_back(tir::transform::CompactBufferAllocation());
  pass_list.push_back(tir::transform::LowerMatchBuffer());
  pass_list.push_back(tir::transform::InjectSoftwarePipeline());
  pass_list.push_back(tir::transform::LowerAtomic());
...
transform::Sequential MixedModulePassManager(IRModule mixed_mod, Target target) {
  ...
  // Move storage rewrite after lower thread allreduce to share allocated shared memory for
  // allreduce.
  if (!disable_storage_rewrite) {
    mixed_pass_list.push_back(tir::transform::StorageRewrite());
  }
  // Move InferFragment below storage rewrite
  mixed_pass_list.push_back(tir::transform::InferFragment());
